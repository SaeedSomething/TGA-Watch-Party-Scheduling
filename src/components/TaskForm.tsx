...
type Props = {
  onUpdate(tasks: TaskSubmission[]): void;
};

export function TaskForm({ onUpdate }: Props) {
  const [name, setName] = useState("");
  const [note, setNote] = useState("");
  const [selectedItem, setSelectedItem] = useState(DEFAULT_TASKS[0]?.id ?? "");
  ...
  const resolvedItem = usingCustom ? customItem : items.find((item) => item.id === selectedItem)?.label ?? "";
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = { name: name.trim(), item: itemLabel, note: note.trim() || undefined };
    ...
    try {
      ...
      setNote("");
      if (usingCustom && customItem.trim()) {
        const newItem: TaskItem = { id: crypto.randomUUID(), label: customItem.trim() };
  ...
  return (
    <form className="glass-card fade-border" onSubmit={handleSubmit} id="task-form">
      <div className="grid two-cols">
        <div className="grid" style={{ gap: "0.4rem" }}>
          ...
          <input
            id="name"
            className="input"
            placeholder="Ari, Sam, Quinn..."
            value={name}
            onChange={(e) => setName(e.target.value)}
          />
        </div>
        <div className="grid" style={{ gap: "0.4rem" }}>
          <label htmlFor="item">What are you covering?</label>
          <select
            id="item"
            className="select"
            value={selectedItem}
      ...
      {usingCustom ? (
        <div className="grid" style={{ gap: "0.4rem", marginTop: "1rem" }}>
          <label htmlFor="custom-item">Describe your custom item</label>
          <input
            id="custom-item"
            className="input"
      ...
      <div className="grid" style={{ gap: "0.4rem", marginTop: "1rem" }}>
        <label htmlFor="note">
          Add a note <span className="subtle">(optional but helpful)</span>
        </label>
        <textarea
          id="note"
          className="textarea"
          placeholder="Specific brand? Time window? Any requests?"
          rows={3}
          value={note}
      ...
      {error ? (
        <p className="subtle" style={{ color: "#fdb0c0", marginTop: "0.4rem" }}>
          {error}
        </p>
      ) : null}
      ...
      <div style={{ marginTop: "1.2rem", display: "flex", gap: "0.75rem", alignItems: "center" }}>
        ...
        <span className="subtle">Your update shows in the list instantly.</span>
      </div>
    </form>
  );
}
